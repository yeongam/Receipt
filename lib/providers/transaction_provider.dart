...
class TransactionProvider extends ChangeNotifier {
  ...
  Future<void> loadMonth(
    ...
    try {
      ...
      final fetched = await _repo.fetchByMonth(
        userId,
        _monthKey(normalizedMonth),
      );
    ...
    final futures = List<Future<MonthlyTrend>>.generate(count, (i) {
      ...
      if (_transactionsByMonth.containsKey(scopedKey)) {
        ...
        final expense = txs
            ...
            .fold(0, (sum, t) => sum + t.amount);
        return Future.value(
          MonthlyTrend(month: dt, income: income, expense: expense),
        );
      }
  ...
  Future<void> _refreshMonthlyTrends(String? userId) async {
    ...
    final capturedGeneration = _generation;
    final trends = await _buildMonthlyTrends(userId, count: _monthlyTrendCount);
    if (_generation != capturedGeneration) return;
    _monthlyTrends = trends;
  }
  ...
  Future<void> deleteTransaction(String id) async {
    ...
    await _repo.delete(id, userId: existing?.userId ?? _currentUserId);
    if (existing != null) {
  ...
  List<AppTransaction> recentTransactions({int limit = 5, DateTime? month}) {
  ...
  List<AppTransaction> transactionsForMonth(DateTime month) {
    ...
    return List.unmodifiable(
      _transactionsByMonth[_scopedKey(userId, month)] ??
  ...
  int totalIncomeForMonth(DateTime month) {
    return transactionsForMonth(
      month,
    ).where((t) => t.isIncome).fold(0, (sum, t) => sum + t.amount);
  }

  int totalExpenseForMonth(DateTime month) {
    return transactionsForMonth(
      month,
    ).where((t) => t.isExpense).fold(0, (sum, t) => sum + t.amount);
  }
